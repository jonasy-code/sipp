...
int main(int argc, char **argv) {
  ...
  tmp = sipp_block(BLOCK_SIZE, BLOCK_SIZE, BLOCK_SIZE / 4.0, &column_surf,
                   granite_shader, WORLD);
  object_move(tmp, 0.0, 0.0, -BLOCK_SIZE / 8.0);
  object_add_subobj(column, tmp);

  for (i = 0; i < NCYL; i++) {
    if (i == 0) {
      tmp = sipp_cylinder(SMALL_CYL_RAD, CYL_LEN, CYL_RES, &column_surf,
                          granite_shader, WORLD);
    } else {
  ...
  spotlight_create(-3.0, -3.0, 6.0, 0.0, 0.0, 0.0, 25.0, 0.45, 0.45, 0.45,
                   SPOT_SHARP, TRUE);

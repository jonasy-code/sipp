...
main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "aj:pgfls:r:c:t")) != EOF) {
        switch (c) {
          ...
          case 'l': mode = LINE;    imfile_name = "geombench.pbm"; break;
          case 's': size = atoi(optarg); break;
          case 'r': res = atoi(optarg);  break;
          case 'c': grid = atoi(optarg); break;
          case 't': report = 1;          break;
        }
    }
    ...
    for (i = 0; i < grid; i++) {
        for (j = 0; j < grid; j++) {
            ...
            *sp = surf;
            torus = sipp_torus(1.0, 0.4, 24 * res, 12 * res, sp,
                               basic_shader, WORLD);
    ...
    camera_params(sipp_camera, 
                  0.0, -1.5 * grid, 2.5 * grid + 3.0,   /* position */
                  0.0, 0.0, 0.0,                         /* look at  */
                  0.0, 1.0, 0.0,                         /* up       */
                  0.5);

...
Object *sipp_ellipsoid(double x_rad, double y_rad, double z_rad, int res,
  ...
  if (texture == SPHERICAL || texture == NATURAL) {
    for (i = 0; i < res; i++) {
      v2[i] =
          (atan(zradprim / sqrt(factor * factor *
                                (x_arr[i] * x_arr[i] + y_arr[i] * y_arr[i]))) /
               M_PI +
           0.5);
    }
    v2[i] = v2[0];
  ...
  for (i = 0; i < res; i++) {
    ...
    case NATURAL:
    case SPHERICAL:
      vertex_tx_push(x_arr[i] * factor, y_arr[i] * factor, zradprim, u_arr[i],
                     v2[i], 0.0);
      vertex_tx_push(factor * x_arr[i + 1], factor * y_arr[i + 1], zradprim,
    ...
    default:
      ...
      vertex_tx_push(factor * x_arr[i + 1], factor * y_arr[i + 1], zradprim,
                     factor * x_arr[i + 1], factor * y_arr[i + 1], zradprim);
    ...
    for (i = 0; i < res; i++) {
      ...
      case SPHERICAL:
        ...
        vertex_tx_push(factor2 * x_arr[i + 1], factor2 * y_arr[i + 1],
                       zradprim2, u_arr[i + 1], v2[i + 1], 0.0);
        vertex_tx_push(factor1 * x_arr[i + 1], factor1 * y_arr[i + 1],
                       zradprim1, u_arr[i + 1], v1[i + 1], 0.0);
        break;
      ...
      case CYLINDRICAL:
        ...
        vertex_tx_push(factor2 * x_arr[i + 1], factor2 * y_arr[i + 1],
                       zradprim2, u_arr[i + 1], zradprim2 / (2.0 * z_rad) + 0.5,
                       0.0);
        vertex_tx_push(factor1 * x_arr[i + 1], factor1 * y_arr[i + 1],
                       zradprim1, u_arr[i + 1], zradprim1 / (2.0 * z_rad) + 0.5,
                       0.0);
        break;
      ...
      default:
        ...
        vertex_tx_push(factor2 * x_arr[i + 1], factor2 * y_arr[i + 1],
                       zradprim2, factor2 * x_arr[i + 1],
                       factor2 * y_arr[i + 1], zradprim2);
        vertex_tx_push(factor1 * x_arr[i + 1], factor1 * y_arr[i + 1],
                       zradprim1, factor1 * x_arr[i + 1],
                       factor1 * y_arr[i + 1], zradprim1);
      }
      polygon_push();
    }
  }
  ...
  for (i = 0; i < res; i++) {
    ...
    case SPHERICAL:
      ...
      vertex_tx_push(x_arr[i] * factor, y_arr[i] * factor, zradprim, u_arr[i],
                     v2[i], 0.0);
      vertex_tx_push(0.0, 0.0, -z_rad, u_arr[i], 0.0, 0.0);
      vertex_tx_push(0.0, 0.0, -z_rad, u_arr[i + 1], 0.0, 0.0);
      break;
    ...
    case WORLD:
    default:
      vertex_tx_push(x_arr[i + 1] * factor, y_arr[i + 1] * factor, zradprim,
                     x_arr[i + 1] * factor, y_arr[i + 1] * factor, zradprim);
      vertex_tx_push(x_arr[i] * factor, y_arr[i] * factor, zradprim,
                     x_arr[i] * factor, y_arr[i] * factor, zradprim);
      vertex_tx_push(0.0, 0.0, -z_rad, 0.0, 0.0, -z_rad);
      break;

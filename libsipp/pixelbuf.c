 ...
 */
static int pixel_alloc(Pixel_buffer *pb);

/*
 * Initialize the pixel buffer.
 */
void pixels_setup(Pixel_buffer *pb, int init_size) {
 ...
 */
void pixels_free(Pixel_buffer *pb) {
  if (pb->pixbuf != NULL) {
    sfree(pb->pixbuf);
  }
  pb->pixbuf = NULL;
 ...
 */
void pixels_reinit(Pixel_buffer *pb) { pb->first_free = 0; }
...
static int pixel_alloc(Pixel_buffer *pb) {
  if (pb->first_free == pb->pixbuf_size) {
    ...
    pb->pixbuf = (Pixel_info *)srealloc(pb->pixbuf,
                                        pb->pixbuf_size * sizeof(Pixel_info));
  }
 ...
 */
int pixel_insert(Pixel_buffer *pb, int pixel, Vector *worldstep,
...
void shade_cache_clear(Pixel_buffer *pb) {
  if (pb->shade_count != NULL) {
    memset(pb->shade_count, 0, pb->shade_npixels * sizeof(int));
  }
}

void shade_cache_free(Pixel_buffer *pb) {
 ...
 */
static bool shade_cache_lookup(Pixel_buffer *pb, int slot, int polygon,
...
static void shade_cache_insert(Pixel_buffer *pb, int slot, int polygon,
                               Color *color, Color *opacity) {
  Shade_entry *e;

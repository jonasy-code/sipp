...
Object *sipp_cone(double radius_bot, double radius_top, double length, int res,
  ...
  if (bot_exists) {
    for (i = res; i > 0; --i) {
      vertex_tx_push(xb[i], yb[i], -half_length, ub[i], vb[i], -half_length);
      vertex_tx_push(xb[i - 1], yb[i - 1], -half_length, ub[i - 1], vb[i - 1],
                     -half_length);
      switch (texture) {
      case NATURAL:
      case CYLINDRICAL:
        vertex_tx_push(0.0, 0.0, -half_length, ub[i - 1], vb[i - 1],
                       -half_length);
        vertex_tx_push(0.0, 0.0, -half_length, ub[i], vb[1], -half_length);
        break;

      case SPHERICAL:
        vertex_tx_push(0.0, 0.0, -half_length, ub[i - 1], 0.0, -half_length);
        vertex_tx_push(0.0, 0.0, -half_length, ub[i], 0.0, -half_length);
        break;

      case WORLD:
      default:
        vertex_tx_push(0.0, 0.0, -half_length, 0.0, 0.0, -half_length);
        break;
      }
      polygon_push();
    }
  ...
  if (top_exists) {
    for (i = 0; i < res; ++i) {
      vertex_tx_push(xt[i], yt[i], half_length, ut[i], vt[i], half_length);
      vertex_tx_push(xt[i + 1], yt[i + 1], half_length, ut[i + 1], vt[i + 1],
                     half_length);
      switch (texture) {
      case NATURAL:
      case CYLINDRICAL:
        vertex_tx_push(0.0, 0.0, half_length, ut[i + 1], vt[i + 1],
                       half_length);
        vertex_tx_push(0.0, 0.0, half_length, ut[i], vt[i], half_length);
        break;

      case SPHERICAL:
        vertex_tx_push(0.0, 0.0, half_length, ut[i + 1], 1.0, half_length);
        vertex_tx_push(0.0, 0.0, half_length, ut[i], 1.0, half_length);
        break;
  ...
  for (i = 0; i < res; ++i) {
    if (top_exists && bot_exists) {
      ...
      case CYLINDRICAL:
      case SPHERICAL:
        vertex_tx_push(0.0, 0.0, half_length, 0.0, 1.0, half_length);
        break;
